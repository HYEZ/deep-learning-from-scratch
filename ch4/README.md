...
- `확률적 경사 하강법(stochastic gradient descent`: 데이터를 무작위로 미니배치로 선정하기 때문
	+ 대부분 딥러닝 프레임워크에선 SGD 함수로 이 기능 구현
...
### 에폭
* 1에폭은 학습에서 훈련데이터를 모두 소진했을 때의 횟수
* ex) 60000개를 100개의 미니배치로 학습할 경우 확률적 경사 하강법을 600회 반복하면 훈련데이터를 모두 소진함, 이 경우 600회가 1에폭
* 평가 시 에폭 단위로 train data와 test data에 대한 정확도 기록